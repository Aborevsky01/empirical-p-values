...
\paragraph{Example 3.} To illustrate the case of class distribution shift, we resampled the test data so that number of true negative and positive instances are equal. This approach was used in other articles \todo{AB}{cite the papers which used rebalanced datasets} \cite{viviano2021saliency}.The accuracy of the CNN classifier (the same as trained in Example \ref{ex:vanilla}) went from 99 \% to xx \% \todo{AB}{get the accuracy}. The evaluation plots are shown in Figure \ref{fig:mnist_rebalance}.
