...
The PatchCamelyon (PCam) benchmark dataset is a collection of histopathologic scans of lymph node sections \todo{AB}{Add citation} derived from the Camelyon 16 challenge \todo{AB}{Add citation}. It consists of 327,680 colored images of size 96x96 each. The challenge is to identify the presence (positive) or absence (negative) of any histopathology. %PCam was deduced from Camelyon16 dataset by splitting images into patches. However, it appears to be far easier in generalization by the neural networks, in turn still showing a high accuracy in metastasis detection on the ancestor. We believe this is a relevant clinical task, which strives to have the control over the possible error rate.
The dataset creators of PCam underline that all the positive and negative instances in the training, validation, test sets are equally distributed, resulting in a class balance of 1:1; while the positive and negative class balance in the Camelyon 16 test dataset is 30.6\% to 69.4 \% . Therefore,  the actual true class proportions might be significantly different in real-life applications, making the FDR control with methods like LTT inaccurate. We manually resampled the test data sets of the PCam so that the class ratios become the same as of the Camelyon, because in our opinion this class ratio is probably closer to the one in real life scenarios.

We used the  resnet34-pcam model \todo{AB}{Add citation, not link}, a popular, deep residual network from the TIA (Tissue Image Analysis) toolbox \todo{AB}{Add citation, not link}.  \todo{AB}{Andrey information about the training pretraining and the accuracies.} %This toolbox for tissue image analysis provides a torch-written API with distinct efficient algorithms for a bunch of various biomedical tasks. Except the trained models themselves, package also introduces tools for data loading, pre- and post-processing, visualization together with the overall inference. 
%For the PCam dataset, TIA package suggest over a dozen of models. We chose the \href{https://huggingface.co/1aurent/resnet34.tiatoolbox-pcam}{resnet34-pcam}. It is the ubiquitously known model from the torchvision deep residual learning collection being further advanced by TIA research centre with the use of PCam data. \todo{AB}{So what is the model we used and what is TIA?} \todo{andrey}{accuracy of model}
%Moreover, the patches do comprise a synthetic beginning since the tumor cells could be found only in the centre region of the image with size of 32x32. %Nevertheless, authors still provide the outer region for model's compliance with Camelyon16, but they do slightly lighten the task by reducing the area of interest.

\begin{figure}
	\centering
	\includegraphics[width=6in]{img/pcam_example.png}
	\caption{{\bf Instances of PCam dataset.} Green boxes in center region indicate a tumor tissue (positive instance), while the lack of a green box indicates a negative tissue. \todo{AB}{Can you make 2x2 negative, left hand side and 2x2 positive instances right hand side. Please remove the green box. no to confuse the reades that the green box is in the original data image. }}
	\label{fig:pcam_example}
\end{figure} 


%XFor the sake of proving practicality of our method, we will manually change the classes' proportions. However, no data distribution shift could be performed except the one laid in test subset.
