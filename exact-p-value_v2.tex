...
For binary classification problems (K=2), let $f:D\rightarrow R$ be a trained discriminator (classifier), $N_0=\{f(t):t\in X,\, Y_t=0\}$ the train null distribution,  $N_0=\{f(t):t\in X,\, Y_t=1\}$ the train target distribution, and $FDP(s_t)=(1+{|\{t: Y_t=0,\,f(t)\ge s_t\}|)/|\{t:f(t)\ge s_t\}|}$. We say that we trust the positive prediction of sample $t$ if and only if $f(t)>s_t$. We note that the FDR control is not related to FSR control in this case. 
...
\begin{figure}
	\centering
	\begin{tabular}{cccc}
		\includegraphics[width=1.5in]{img/Mnist_multi_score_dist.pdf}&
		\includegraphics[width=1.5in]{img/cnn_QQ_multi.pdf} &
		\includegraphics[width=1.5in]{img/cnn_multi_fdr_control.pdf} &
		\includegraphics[width=1.5in]{img/tg_image_2497405997.jpeg} \\
