...
%    
%    
%    The neural network is trained in a classic manner on training data $X^{\text{train}}$, regardless of the classification type (binary or multiclass).
...
%
%The mean and standard deviation of both $P_{\text{ref}}$ and $N_{\text{ref}}$ are then calculated. After, we do substitute $\mu_{N_{\text{p}}}$ and $\sigma_{N_{\text{p}}}$ by the reference ones ones. In the case of a positive $P_p$, only mean statistics is modified.
%    \begin{equation}
