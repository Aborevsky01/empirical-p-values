...
When the training null distribution $X_0$ is different from the test null distribution $T_0$, then the EPVs will not be uniformly distributed and the BH procedure results in liberal or conservative bias in FDR estimation. In the following steps, we describe the TNA method that adjusts the prediction scores so that the approximated test null distribution $T_0$ aligns the actual training null distribution $X_0$.

\begin{enumerate}%[label=\arabic*.]
	\itemsep-3pt  		
	\item Get the count histograms of $X_0$, $X_s$, and $T_s$ distributions with identical bin borders and denote them as $H_0$, $H_X$, and $H_T$, respectively. $H[i]$ denotes the count in bin $i$.  We used 100 bins in our experiments and it worked fine.
	

	\item We create a new histogram $\hat{T_0}$ that will be an approximation of the unknown score distribution of the negative test samples $T_0$ (test null). The counts of the negative test samples in $\hat{T_0}[i]$ will be estimated by $\hat{T_0}[i] = H_T[i]\cdot \frac{H_0[i]}{H_X[i]}$. This estimation assumes that the ratio of the positive and negative instances remain the same in the training and test data.
	
	\item Let $\pi_X$ be the proportions of negative samples in the training data. This can be calculated with the true training class labels. Let $\pi_T$ be the proportions of the negative predictions among all test predictions. This can be calculated with the predicted class labels. Finally, Let $\tau=\pi_T/\pi_X$. 
	
	%The counts of the negative test samples in $\hat{T_0}[i]$ will be estimated by $\hat{T_0}[i] = H_T[i]\cdot \frac{H_0[i]}{H_N[i]}$.
	 
	 
%	the $\hat{T_0}[i]=T[i]\cdot c_i\cdot$, where $c_i=H_0[i]/H_N[i]$ is the proportion of negative training samples in bin $i$.

	
	%\item Let us create a new histogram $\hat{T_0}$, which will be an approximation of the unknown test null distribution $T_0$. To estimate the counts of the negative $\hat{T_0}[i]=T[i]\cdot c_i\cdot$, where $c_i=H_0[i]/H_N[i]$ is the proportion of negative training samples in bin $i$.
