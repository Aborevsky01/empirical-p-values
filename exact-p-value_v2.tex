...
However, when we run the tests on a single images, \todo{AB}{come on!} 
...
The Breast Cancer Semantic Segmentation (BCSS) dataset \cite{Amgad2019StructuredCE} contains around 20,000 image segmentation images annotated with Digital Slide Archive, with a particular focus on tissue regions derived from breast cancer imagery obtained from The Cancer Genome Atlas (TCGA). The image pixels are labeled by: 'Stroma', 'Necrosis', 'Tumor' and 'Others', and the corresponding class ratios in both the training and test sets are: X,Y,V,W\todo{AB}{Get the ratios}. We used the deep learning model called \texttt{'fcn resnet50 unet-bcss'} from the TIA (Tissue Image Analysis) toolbox \cite{Pocock2022} to classify the pixels to one of the four classes. The model achieved an 84 \% and 82 \% accuracies on the training and test datasets. 
...
%	\advance\leftskip-0.5cm
	...
	\caption{{\bf An instance from CheXpert dataset.}\todo{AB}{Could oyu please put the 4 images in one row, just like for the PCam or TissueNet.}}
