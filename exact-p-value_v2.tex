...
\begin{enumerate}%[label=\arabic*.]
	...
	\item Let $\hat{\pi}_0$ be the proportions of the negative test predictions. This is approximated with using the number of positive and negative test predictions. 
	
	\item We create a new density histogram $\hat{T}_0$ that will be an approximation of the test null $T_0$. The value of the histogram bin $i$ is calculated  by $\hat{T}_0[i] = (\overline{T}_s[i] - (1-\hat{\pi}_0)\cdot \overline{X}_1[i])/\hat{\pi}_0$. Note that, the unknown $T_1$ is approximated by $X_1$ here. 
	
	\item We polish the distribution approximation a bit. Let $k$ be the smallest bin index, where $\hat{T}_0[k] <0$ and then let $\hat{T}_0[l] =0$ for $l\ge k$. 
