...
\title[Article Title]{Toward reliable false discovery rate control in classification problems under distribution shift}


\author[1]{\fnm{Andrey} \sur{Borevskiy}}\email{aoborevskiy@hse.ru}

\author*[1]{\fnm{Attila} \sur{Kertesz-Farkas}}\email{kfattila@yandex.com}


\affil*[1]{\orgdiv{Laboratory on AI for Computational Biology, Faculty of Computer Science}, \orgname{HSE University}, \orgaddress{\street{11 Pokrovsky Bvld}, \city{Moscow}, \postcode{109028}, \country{Russian Federation}}}

\abstract{\textbf{Background:} Domain shifts or batch effects can significantly degrade the performance of machine learning-based classifiers in software production without any warning. This can particularly be problematic in medical applications which involve human lives. The shift changes the shape of the prediction score distribution of the test samples compared to that of the training samples; hence, decision boundary re-calibration would be required to conform to the expected performance standards. 

\textbf{Results:} We present a simple, yet robust heuristic method to calculate valid p-values (uniformly distributed) for raw prediction scores of test samples under data shift in general classification problems. Our method is based on an adjustment of the test prediction scores so that the test null distribution (approximated distribution of the prediction scores of negative test samples) aligns to the training null distribution (actual distribution of prediction scores of the negative training samples). We termed it the Test Null Adjustment (TNA) method. Then, empirical p-values of the adjusted test prediction scores can be calculated with respect to the training null distribution to be used in any statistical applications which require test samples accompanied by valid p-values. We demonstrate the usability of our TNA method in FDR control with Benjamini-Hochberg (BH) protocol. 
...
\keywords{Empirical P-values, False Discovery Rate, Data Shift, Batch Effect}
...
\begin{equation*}
	FDP(s_t,K)=\frac{|\{t\in K: Y_t=0,\,f(t)\ge s_t\}|}{|\{t\in K:f(t)\ge s_t\}|},
\end{equation*}
...
Not applicable. This study did not involve the recruitment of human participants or the collection of new human-related data.

\subsection{Consent for publication}
Not applicable. Manuscript contains no individual person's data in any form. 
...
Funding of the research was fully conducted by HSE University.

\ifdefined\DOUBLEBLINDREVIEW
Omitted for double-blind peer-review.
\else
\subsection{Authors' contribution}
AKF conceived the idea and supervised the project,  AB worked out the methods and carried out experiments. AKF and AB wrote the manuscript.
\fi 
