...
\response{We agree, and apologize for not making this earlier. Our implementation code is now publicly available at \url{https://github.com/Aborevsky01/empirical-p-values}, without restriction. We have added a statement to the manuscript pointing to this repository.}
...
\edit{In our opinion, the comparatively weaker performance of the TNA method on the BCSS dataset stems from subpopulation shift (proportions of different subclasses change). The shapes of the modes of the training and test null distributions differ more substantially than in the other benchmark datasets. In such cases, a simple affine adjustment of the test scores cannot adjust the test null mode close to the training null mode. The differing shapes of the modes across the three classes can be seen in Fig \ref{fig:BCSS-classwise}. This divergence between the training and test null modes is most pronounced for the Tumor class.}
...
\response{More fundamentally, we think that a formal error bound cannot be derived without committing to some specific assumption on how the test distribution may differ from the training distribution (e.g., a bound on the Wasserstein distance or the Jensen-Shannon divergence between $X_0$ and $T_0$); without such a restriction, the test null could in principle differ arbitrarily from the training null, and no method could guarantee accurate FDR control from a finite sample alone. This is not specific to TNA: standard hypothesis testing routinely assumes, for instance, that the null distribution is exactly Gaussian, an assumption that is essentially never exactly satisfied in practice, yet this discrepancy is broadly accepted because the resulting approximation is useful in practice. We therefore view the absence of an assumption-free theoretical guarantee as an inherent feature of the problem itself, rather than a specific shortcoming of TNA, and we have chosen to state this limitation explicitly rather than overclaim the theoretical guarantees of TNA.}
