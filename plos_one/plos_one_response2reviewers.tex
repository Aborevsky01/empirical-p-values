...
\response{Thank you. We have checked the manuscript against the PLOS ONE formatting templates (main body and title/authors/affiliations) and adjusted the file naming of the manuscript and figure files accordingly for resubmission. \fixme{Please confirm the final file names used on resubmission (Manuscript, Figures, Revised Manuscript with Track Changes) follow the PLOS ONE convention before uploading.}}
...
\response{We agree, and apologize for not making this explicit earlier. Our implementation code is publicly available at \url{https://github.com/Aborevsky01/empirical-p-values}, without restriction. We have added a statement to the manuscript pointing to this repository, and we are supplementing it with an environment specification (\texttt{requirements.txt}/\texttt{environment.yml}) and a short README describing how to reproduce the experiments, as also requested by Reviewer 2 below (point 3). \fixme{Add the code-availability sentence to the manuscript (e.g., in Materials and methods or as a new ``Code availability'' note) once the repository's README/environment file is finalized.}}
...
\response{Thank you for flagging this. The manuscript already lists, for each of the five datasets used (MNIST, BCSS, CheXpert, PCam, TissueNet), the dataset name together with its DOI and/or URL in Table 1 (``Data Availability Statement containing dataset names, repositories, and accession details''). \fixme{The separate Data Availability Statement field in the submission system appears to be empty/incomplete; we will populate it with the same repository names and DOIs/URLs already listed in Table 1 at resubmission.}}

4. We note that you have included the phrase ``data not shown'' in your manuscript.

\response{Thank you for catching this. The phrase referred to an exploratory comparison with one-dimensional optimal transportation (OT), which is not a core part of our study; we have therefore removed the reference to unshown data and rephrased the sentence instead of adding a citation or a new repository entry for it. The revised sentence now reads:}

\edit{We also note that we tested a one-dimensional optimal transportation (OT) approach to adjust the training null to the test null. Unfortunately, this approach is very sensitive to the test null approximation, especially at the decision boundary. In our exploratory experiments, OT severely overfitted, while TNA remained more robust to poor tail approximation of the null distributions; consequently, we did not pursue OT further as a baseline in this study.}
...
\response{We have reviewed our reference list and, to the best of our knowledge, none of the cited works have been retracted. \fixme{Please double-check the bibliography (bibliography.bib) once more against the Retraction Watch database before final submission, since this is best verified by the corresponding author.}}
...
\response{Thank you for pointing this out. Our implementation code is hosted at \url{https://github.com/Aborevsky01/empirical-p-values}, which is publicly accessible. We acknowledge that, at the time of submission, this repository did not include an explicit environment specification. We are adding a \texttt{requirements.txt}/\texttt{environment.yml} file pinning the package versions used (Python, NumPy, SciPy, scikit-learn, PyTorch/TensorFlow as applicable) together with a README describing how to reproduce each experiment in the manuscript, and we are adding a corresponding code-availability statement to the manuscript. \fixme{Finalize the environment file and README in the repository, then add a one-sentence code-availability statement to the manuscript (Materials and methods) citing the repository URL, before resubmission.}}
