...
The False Selection Rate (FRS) control procedure can restrain the FP and FN errors simultaneously \cite{zhao2023controlling}. This approach determines two thresholds, one to make definitive positive, and another one to make definitive negative predictions. Any samples not passing either thresholds are remain in indecision. Therefore, the misclassification error (accuracy) among definitive predictions can be bounded, while a subset of samples without predictions are returned for manual investigation by a human expert or for further data collection \cite{rava2021burden}. 

Typically, the FDR (and the FSR) control heavily relies on valid, i.e. uniformly distributed, p-values, while typical deep learning models produce raw discriminative scores, logits. Recent methods in outlier detection \cite{bates2023testing,marandon2023adaptive} and in classification \cite{rava2021burden, angelopoulos2021learn} have proposed using empirical p-values, also known as conformal p-values, in which a p-value of a given discriminative score $s$ is defined as the fraction of a set of validation  scores that are larger than or equal to $s$, formally: $p=(1+\sum_{s_i\in C} \mathds{1}\{s>s_i\})/(n+1)$. The set $C$ denotes a set of scores obtained with a validation set, which can be constructed from the labeled training data or it can be created from the decoy peptides in tandem mass spectrometry data annotation \cite{elias2007target,danilova2019bias}. The p-values of test samples are super uniform under the null if the data distribution of the test and validation samples are identically and independently distributed. This property is called exchangeability. In this case, the Benjamini-Hochberg (BH) procedure controls the FDR at $\pi_0\alpha$ level, where $\pi_0$ denotes the fraction of nulls among the test samples, and $\alpha$ is a user-defined confidence level. However, in practice, the exchangeability is often violated due to data shifts, batch effects, as discussed above; thus, the FDR control may result in either liberal or conservative bias. 

Most of the methods have been developed to detect covariate shifts in the feature space \cite{kulinski2020feature, budhathoki2021did}, these methods often challenged by high-dimensional data, data sparsity, and correlated features. Label shift can be cby \cite{lipton2018detecting} can be circumvented by decision threshold readjustment. blablb

In this paper we show a very simple method to detect 
...
We note that several algorithms have been designed to handle data distribution shifts, concept drifts within the training data; however, this case is not the topic of this paper.
