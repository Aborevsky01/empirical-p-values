...
The performance of a machine learning-based classifier in the software production phase is assumed to be the same as its performance on a validation set. Unfortunately, the real performance can be significantly worse than thought due to domain shifts, also known as data distribution shift, class label distribution shift, or batch effects. In life sciences, this shift in data can be attributed including, but not limited, to laboratory and environmental conditions, change in the reagent, atmospheric ozone levels, personnel differences \cite{leek2010tackling}. Different instruments may also produce slight differently distributed data about the same experiments. Light microscopic images may vary due to the ambient light conditions. These effects may lead to incorrect conclusions about the experiments. Machine learning-based methods for credit risk assessment become inaccurate after economic or geopolitical turbulence. 

Type I and type II errors can have significantly different costs or impacts on society. An incorrectly diagnosed HIV or COVID positive person will undergo a more elaborated clinical tests until the correct diagnosis established, while an incorrectly diagnosed infected person will put many other peoples' life in danger.
